<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'our_college/cityyear')) ) {
		...
		$register_my_attendance_for = (is_array($_POST['register_my_attendance_for'])) ? implode('; ', $_POST['register_my_attendance_for']) : $_POST['register_my_attendance_for'];
		$register_my_attendance_for = mysql_real_escape_string($register_my_attendance_for, $sql->Link_ID);
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_role_models  
		(register_my_attendance_for, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, receive_communications, datetime_submitted) 
		VALUES('$register_my_attendance_for', '$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/our_college/cityyear?registration=failed');
			exit;
		}
