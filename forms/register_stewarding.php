<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'event_stewarding')) ) {
		...
		$date_of_birth = mysql_real_escape_string($_POST['date_of_birth'], $sql->Link_ID);
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_stewarding_dec_2010 
		(firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/our_courses/event_stewarding?registration=failed');
			exit;
		}
