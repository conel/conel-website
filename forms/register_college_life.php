<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'our_college/college_newsletter')) ) {
		...
		$delivery_preference = mysql_real_escape_string($_POST['delivery_preference'], $sql->Link_ID);
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_college_life 
		(delivery_preference, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$delivery_preference', '$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/our_college/college_newsletter?email=failed');
			exit;
		}
