<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'for_learners/open_days')) ) {
		...
		// nkowald - 2012-01-23 - Separated this into Tottenham and Enfield
		$register_for_tottenham = (is_array($_POST['register_for_tottenham'])) ? implode('; ', $_POST['register_for_tottenham']) : $_POST['register_for_tottenham'];
		$register_for_tottenham = mysql_real_escape_string($register_for_tottenham, $sql->Link_ID);
		
		$register_for_enfield = (is_array($_POST['register_for_enfield'])) ? implode('; ', $_POST['register_for_enfield']) : $_POST['register_for_enfield'];
		$register_for_enfield = mysql_real_escape_string($register_for_enfield, $sql->Link_ID);
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_open_day_2012_13 
		(tottenham_open_day, enfield_open_day, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, how_heard, receive_communications, datetime_submitted) 
		VALUES('$register_for_tottenham', '$register_for_enfield','$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$how_did_you_hear_about_us', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/for_learners/open_days?registration=failed');
			exit;
		}
