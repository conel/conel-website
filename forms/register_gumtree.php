<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'gumtree')) ) {
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_gumtree (message, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		...
		if ($added) {
				...
				header('Location: http://www.conel.ac.uk/gumtree/enquiry_sent');
				exit;			
		} else {
			echo '<p>Failed to add info to database</p>';
			header('Location: http://www.conel.ac.uk/gumtree?registration=failed');
			exit;
		}
