<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'get_a_course_guide')) ) {
		...
		$which_course_guide = (is_array($_POST['which_course_guide'])) ? implode('; ', $_POST['which_course_guide']) : $_POST['which_course_guide'];
		$which_course_guide = mysql_real_escape_string($which_course_guide, $sql->Link_ID);
		
		$print_copy = mysql_real_escape_string($_POST['print_copy'], $sql->Link_ID);
		$print_copy = ($print_copy == 'Yes') ? 'Yes' : 'No';
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_course_guide (which_course_guide, print_copy, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$which_course_guide', '$print_copy', '$firstname','$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added === TRUE) {
			...
			// nkowald - 2011-03-29 - Doesn't need to send email
			...
			$body_html .= '<p>You have received a new \'Course Guide\' form submission.<br /><br />Submitted: '.$date_now.'</p>';
			...
			$mail->Subject = "Download a Course Guide";
			...
			*/
			...
			// Everything went well, let's give these people what they want.
			$_SESSION['course_guide'] = TRUE;
			$_SESSION['selected_guides'] = array();
			if (isset($_POST['which_course_guide']) && is_array($_POST['which_course_guide'])) {
				foreach($_POST['which_course_guide'] as $guide) {
					$_SESSION['selected_guides'][] = $guide;
				}
			}
			if ($_SESSION['course_guide'] == TRUE) {
				header('Location: http://www.conel.ac.uk/our_courses/get_a_course_guide/course_guides');
				exit;
			}
			...
			header('Location: http://www.conel.ac.uk/our_courses/get_a_course_guide?email=failed');
			exit;
		}
