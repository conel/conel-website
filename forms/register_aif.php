<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'february_2011/apprenticeship_fairs')) ) {
		...
		// Convert apprenticeships fair to book for to CSV
		$apprenticeship_fair_booking_for = (is_array($_POST['apprenticeship_fair_booking_for'])) ? implode('; ', $_POST['apprenticeship_fair_booking_for']) : $_POST['apprenticeship_fair_booking_for'];
		$apprenticeship_fair_booking_for = mysql_real_escape_string($apprenticeship_fair_booking_for, $sql->Link_ID);
		
		// Convert which apprenticeships to CSV
		$apprenticeship_of_interest = (is_array($_POST['apprenticeship_of_interest'])) ? implode('; ', $_POST['apprenticeship_of_interest']) : $_POST['apprenticeship_of_interest'];
		$apprenticeship_of_interest = mysql_real_escape_string($apprenticeship_of_interest, $sql->Link_ID);
		
		// Convert courses of interest to CSV
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_apprenticeship_information_fairs  
		(firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, apprenticeship_fair_booking_for, apprenticeship_of_interest, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$apprenticeship_fair_booking_for', '$apprenticeship_of_interest', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/news_events/events_calendar/february_2011/apprenticeship_fairs?registration=failed');
			exit;
		}
