<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'for_employers/apprenticeship_vacancies')) ) {
		...
		$vacancy_number = mysql_real_escape_string($_POST['vacancy_number'], $sql->Link_ID);
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_apprenticeship_vacancies
		(vacancy_number,firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, how_did_you_hear_about_us, receive_communications, datetime_submitted) 
		VALUES('$vacancy_number', '$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$how_did_you_hear_about_us', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://http://www.conel.ac.uk/for_employers/apprenticeship_vacancies?registration=failed');
			exit;
		}
