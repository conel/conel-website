<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'], 'our_courses/kickstart_your_career')) ) {
		...
		// Convert 'which short courses are you interested in' to CSV values
		$courses_interested_in = (is_array($_POST['courses_interested_in'])) ? implode('; ', $_POST['courses_interested_in']) : $_POST['courses_interested_in'];
		$courses_interested_in = mysql_real_escape_string($courses_interested_in, $sql->Link_ID);
		...
		// nkowald - 2011-03-09 - Adding which page sent the form

		// Build INSERT query
		$query = "INSERT INTO tbl_kickstart   
		(courses_interested_in, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, what_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$courses_interested_in', '$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			<html>
			<head><title>Kickstart for College and for your career Registration</title>
			<style type="text/css">
			body, table td {
			...
			<body>';
			$body_html .= '<p>You have received a new \'Kickstart for College and for your career\' form submission.<br /><br />Submitted: '.$date_now.'</p>';
			...
			// Set Recipient
			$mail->AddAddress('employerinfo@staff.conel.ac.uk ', 'Employer Information');
			//$mail->AddBCC('NKowald@staff.conel.ac.uk', 'Nathan Kowald');
			$mail->Subject = "Kickstart form submission";
			...
			if ($result) {
				...
				header('Location: http://www.conel.ac.uk/our_courses/kickstart_your_career?email=failed');
				exit;
			}
			...
			header('http://www.conel.ac.uk/our_courses/kickstart_your_career?registration=failed');
			exit;
		}
