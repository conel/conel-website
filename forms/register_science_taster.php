<?php
	...
	if (isset($_POST['firstname']) && (isset($_SERVER['HTTP_REFERER']) && strpos($_SERVER['HTTP_REFERER'],'march_2011/science_taster_sessions')) ) {
		...
		$register_my_attendance_for = mysql_real_escape_string($_POST['register_my_attendance_for'], $sql->Link_ID);
		$science_taster_enfield = isset($_POST['science_taster_enfield']) ? mysql_real_escape_string($_POST['science_taster_enfield'], $sql->Link_ID) : '';
		$science_taster_tottenham = isset($_POST['science_taster_tottenham']) ? mysql_real_escape_string($_POST['science_taster_tottenham'], $sql->Link_ID) : '';
		...
		// Build INSERT query
		$query = "INSERT INTO tbl_science_taster   
		(register_my_attendance_for, science_taster_enfield, science_taster_tottenham, firstname, surname, date_of_birth, email, telephone_landline, telephone_mobile, address_line_1, address_line_2, address_line_3, postcode, borough_county, which_secondary_school_did_you_attend, courses_of_interest, receive_communications, datetime_submitted) 
		VALUES('$register_my_attendance_for', '$science_taster_enfield', '$science_taster_tottenham', '$firstname', '$surname', '$date_of_birth', '$email', '$telephone_landline', '$telephone_mobile', '$address_line_1', '$address_line_2', '$address_line_3', '$postcode', '$borough_county', '$what_secondary_school_did_you_attend', '$courses_of_interest', '$receive_communications', '$date_now_mysql')";
		...
		if ($added) {
			...
			*/
			...
			header('Location: http://www.conel.ac.uk/news_events/events_calendar/january_2011/open_days?registration=failed');
			exit;
		}
