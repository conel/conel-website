...
class FNOutput{
	...
	// nkowald - 20/0709 - Need to pass session variable through all links as accessed via webmatrix
	var $session_var;
	...
	function FNOutput(){
		...
		// nkowald - 20/07/09
		$this->session_var = (isset($_GET['Session']) && $_GET['Session'] != '') ? $_GET['Session'] : '';
	}
	...
	function folderList($folders){
		...
		for($i = 0; $i<count($folders);$i++){
			...
			if(!in_array($hPath,$this->ignoreFolders)){
				...
				$comment = "<span class=\"folder_comment\"> <a href=\"$PHP_SELF?dir=$hPath&amp;Session=$this->session_var\"> ></a>";
	...
	<li class=\"$class1\" id=\"".$this->stripID("li_$path/$file")."\">
		<!-- FOLDER TITLE -->
		<a href=\"?dir=$path&amp;Session=$this->session_var\" class=\"".$this->stripID("$path/$file")." $path\" title=\"click to open this folder\">$file</a> <span>$comment</span>
		<!-- START CONTENTS -->
	...
	function fileList($files){
		...
		for($i = 0; $i<count($files);$i++){
			...
			if(!in_array($hPath,$this->ignoreFiles) && $file != "fComments.txt"){
				...
				<li class=\"$class1\">
					<!-- FILE TITLE -->
					".$this->doFileLink($file,$path)." <a href=\"?dir=".$this->stripID("F_$path/$file")."&amp;Session=$this->session_var\" class=\"properties ".$this->stripID("$path/$file")."\" title=\"show properties\"><span>View $file Properties</span></a>
					<!-- START PROPERTIES -->
	...
	function doFileProps($file,$path){
		...
		if(in_array(strtolower($ext),$this->imgTypes)){
			...
			<dt>Options:</dt>
			<dd><a href=\"$PHP_SELF?src=$path&amp;Session=$this->session_var\" title=\"view $file source code\">view source</a></dd>";
	...
	function doFileLink($file,$path){
		...
		$ext = substr(strrchr($file, '.'), 1);
		if(in_array(strtolower($ext),$this->imgTypes)){
			return "<a href=\"$PHP_SELF?view=$path&amp;Session=$this->session_var\" title=\"view $file\">$file</a>";
		}else if(in_array(strtolower($ext),$this->embedTypes)){
			return "<a href=\"$PHP_SELF?view=$path&amp;Session=$this->session_var\" title=\"view $file\">$file</a>";
		}else if(in_array(strtolower($ext),$this->phpTypes) || in_array(strtolower($ext),$this->htmlTypes)){
	...
	function viewFile($file){
		...
		if(in_array($ext,$this->imgTypes)){
			...
			echo("\n<div id=\"imgWrapper\">\n<div id=\"imgPreview\">");
			if($preNext[0] != ""){
				echo("\n<a href=\"$PHP_SELF?view=".$preNext[0]."&amp;Session=".$this->session_var."\">Prev</a>");
			}
			if($preNext[0] != "" && $preNext[1] != ""){
				echo(" | ");
			}
			if($preNext[1] != ""){
				echo("\n<a href=\"$PHP_SELF?view=".$preNext[1]."&amp;Session=".$this->session_var."\">Next</a>");
			}
			echo "\n<br /><br />";
			if($preNext[1] != ""){
				echo("\n<a href=\"$PHP_SELF?view=".$preNext[1]."&amp;Session=".$this->session_var."\"><img src=\"".$_GET['view']."\" width=\"".$imageDim[0]."\" height=\"".$imageDim[1]."\" alt=\"".$_GET['view']."\" /></a>\n<br /><br />");
			}else{
				echo("\n<img src=\"".$_GET['view']."\" /><br /><br />");
			}
			echo("\n<a href=\"$PHP_SELF?view=".$this->getFolder($file)."&amp;Session=".$this->session_var."\" title=\"close image\">close</a>");
			echo("\n</div>\n</div>\n<br /><br />
			...
			echo("<div id=\"imgWrapper\"><div id=\"imgPreview\"><embed autoplay=\"false\" src=\"".$_GET['view']."\" ".$dimensiones[3]."></embed></div></div><br /><br />".$_GET['view']."<br />
			last changed: " . date($this->dateFormat, filectime($_GET['view'])) . "<br />
			size: " . $this->returnFileSize(filesize($_GET['view'])) . "
			<br /><br />");
		}
