...
var session_var = getURLVar('Session');
...
com.filenice.actions = function(){
	...
	this.getFolder = function(){
		...
		if(element.className == "contents") {
			startBusy();
			var url="file_browser.php?action=getFolderContents&dir="+dir+"&Session="+session_var;
...
function getNextImage(currentPic){
	var url="file_browser.php?action=nextImage&pic="+currentPic+"&Session="+session_var;
	if (window.XMLHttpRequest){ // moz
		request = new XMLHttpRequest();
	}else if (window.ActiveXObject){ // IE
		request = new ActiveXObject("Microsoft.XMLHTTP");
