<?php
	...
	function module_getGlobals($id,$params) {
		...
		$footer_code = '<script type="text/javascript">
		...
		return array('BANNER'=>$banner, 'ENFIELD_CSS'=>$enfield_css, 'ENFIELD_INC'=>$enfield_inc, 'FOOTER_CODE'=>$footer_code, 'STAFF_BBQ_CONTENT'=>$staff_bbq_content);
	}
		...
		while($db->next_record(MYSQL_ASSOC)) {		
			...
			if (!checkFlag($page["flags"],PAGE_INVISIBLE) && checkFlag($page["flags"],PAGE_ACTIVE)) {
				...
				if ($current_page == 2274) {
					...
					$thumbnail = ($filename != '') ? '<a href="'.getTargetURL($current_page).'"><img src="/images/'.$filename.'" alt="" width="55" height="55" /></a>' 
					: 
					'<a href="'.getTargetURL($current_page).'"><img src="/layout/img/icon_calendar.jpg" alt="" width="55" height="55" /></a>';
				}
				//$no_chars = 42;
