...
function sort_module_byname() {
	...
	$method = array_search('byname',$_PAGE_SORTOPITIONS);

	if(!isset($_GET['cmd'])) {
		$actionURL = 'admin.php?op=update_menu_order&menu_id='.$menu_id.'&sortmethod='.$method.'&cmd=dosort';
		sort_module_byname_output_progress(LANG_Sort_ByName_Name,LANG_Sort_ByName_ProgressDesc,$actionURL);
	} else {
		sort_module_byname_doSort($menu_id);
		## now update the pagemenu
		ui_output_confirm(LANG_Sort_ByName_SuccessDesc,'',"matrix_menu.php");
	}
}
...
##  
## ======================================================================= 
function sort_module_byname_output_progress($title,$message,$target) {
	global $gSession;
	## prepare the template file
	$select_template = new Template('sort/byname/interface/');
...
##  TODO: 
## ======================================================================= 
function sort_module_byname_doSort($menu_id) {
	global $Auth;
