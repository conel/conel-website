...
## =======================================================================        
function page_selector_drawMenu($vTree,$vVarsToBeSet,$vTargetURL) {
	global $gSession,$Auth;

	##var_dump($vVarsToBeSet);
	## first we set the vars
	$tree = $vTree;
	$p = $_GET['p'];
	
	## prepare the template file
	$layout_template = new Template(ENGINE."datatypes/extra_linkback/interface");
	$layout_template->set_templatefile(array("head" => "page_selector.tpl","spacer" => "page_selector.tpl","expand" => "page_selector.tpl","element" => "page_selector.tpl","lastelement" => "page_selector.tpl","foot" => "page_selector.tpl"));

	$layout_template->set_var("Title",LANG_LINKLIST_SelectTitle);
	$layout_template->set_var("Desc",LANG_LINKLIST_SelectDesc);
 	...
 	if(isset($explevels)) {
		...
		for ($i=0; $i < count($explevels); $i++) {
			...
			if (($visible[$n]==1) && ($expand[$n]==1)) {
				$j=$n+1;
				while (isset($tree[$j]["level"]) && $tree[$j]["level"] > $tree[$n]["level"]) {
	...
	$layout_template->set_var('Session',$gSession->id);
	$layout_template->set_var('op',"store");
	##$layout_template->set_var('low_sub',$low_sub);	
	$layout_template->set_var('item_id',$vVarsToBeSet["ItemID"]);	

    ## finally we set the the passed vars 
    if(is_array($vVarsToBeSet)) {
		while(list($key,$val) = each($vVarsToBeSet)) {
			## loop through all the entries
			$layout_template->set_var($key,$val);
		}
	}
	...
	while ($cnt<=count($tree)) {
		if (isset($visible[$cnt]) && $visible[$cnt]) {
			$id++;
			if(isset($vVarsToBeSet["targetPageID"]) && $tree[$cnt]["page_id"] == $vVarsToBeSet["targetPageID"]) {
				## start new row 
      			echo '<tr bgcolor="#96A4CC">';
			} else {
				echo "<tr>";
			}
			...
			/********************************************/
			if (isset($tree[$cnt+1]["level"]) && $tree[$cnt+1]["level"] > $tree[$cnt]["level"]) {
				/****************************************/
			...
			/* output item                          */
			/****************************************/
			$url = "editor.php?op=store&selected_page=".$tree[$cnt]["page_id"];
			$url = $gSession->url($url);
			
      		$layout_template->set_var("id",$id+1);
      		$layout_template->set_var("pageid",$tree[$cnt]["page_id"]);
