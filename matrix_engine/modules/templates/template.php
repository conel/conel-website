...
switch($op) {            
	...
	case "save_template":
		...
		## we need to confirm that the page was saved
		## and we need to refresh the menu on the left
		ui_output_confirm(LANG_TemplateSaved, LANG_TemplateSavedSuccessfull, "menu.php");
	...
	case "update_template":
		...
		ui_output_confirm(LANG_TemplateChanged, LANG_TemplateChangedSuccessfull, "menu.php");
		break;	
	case "delete_template":
		## as the user if he really wants to delete this template
 		$hiddenfields = array('op'=>'doDelete','template_id'=>$template_id);
 		ui_output_Dialog('Template','Do you really want to delete the selected template','template.php',$hiddenfields); 	
		break;	
	case "doDelete":
		## here we actually delete the template
		delete_template($template_id);
		ui_output_confirm(LANG_TemplateDeleted, LANG_TemplateDeletedSuccessfull, "menu.php");
		break;	
	case "newGroup":
 		## prepare the hidden fields
 		$hiddenfields = array('op'=>'DoNewGroup');
 		ui_output_InputDialog(LANG_MODULE_templates_NewGroup,LANG_MODULE_templates_NewGroupDesc,'template.php',$hiddenfields); 	
		break;	
	case "DoNewGroup":
 		## okay now we need to prepare the data and actually create a new group entry
 		$newgroupname = addslashes($_POST['value']);
 		$group_id =  new_group($newgroupname);
 		ui_output_confirm(LANG_MODULE_templates_NewGroup, LANG_MODULE_templates_NewGroupSuccess, "menu.php");
		break;		
	default:
		...
		ui_output_error("<b>".LANG_TemplateEditor."</b><br>".LANG_TemplateDescription);
		break;
}
