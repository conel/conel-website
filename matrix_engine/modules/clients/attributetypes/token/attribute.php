...
##
## =======================================================================        
function clients_token_displayInput($ctlData,$id) {
	global $Auth,$gSession;
	
	## check if we need to setup this attribute type
	clients_token_setup($ctlData['IDENTIFIER']);
	
	## then we need to get any data that was previously entered (not yet)
	$value = clients_token_getData($ctlData,$id);
	$value = htmlentities($value);
	## and finally we output the input form
	## prepare the template
	$inputFile = "input.tpl";
	$template = new Template(ENGINE.'modules/clients/attributetypes/token/interface');
...
## =======================================================================        
##  clients_token_deleteData        
## =======================================================================        
##  retrieves the data for a certain field of a certain client
##
## =======================================================================        
function clients_token_deleteData($ctlData,$clientID) {	
...
## =======================================================================        
##  clients_token_getData        
## =======================================================================        
##  retrieves the data for a certain field of a certain client
##
## =======================================================================        
function clients_token_getData($ctlData,$clientID) {	
	## db object
...
function clients_token_storeInput($ctlData,$clientID) {
	...
	if(isset($_POST[$ctlData['IDENTIFIER']]) && !empty($_POST[$ctlData['IDENTIFIER']])) {
		## we need to prepare the input - needs to be done properly
		$data = $_POST[$ctlData['IDENTIFIER']];
		$data = mysql_real_escape_string($data);
	} else {
		## we need to check if we already have a token
		$data = clients_token_getData($ctlData,$clientID);
		
		if(empty($data)) {
			## if not we generate a new token
			$data = substr(md5(uniqid('').getmypid()),0,10);
		}
	}
...
##
## =======================================================================        
function clients_token_validateInput($ctlData,$input_data = null) {
	global $Auth,$gSession;
...
##
## =======================================================================        
function clients_token_setup($identifier) {
	...
	if(!$exists) {
		...
		$query = 'ALTER TABLE '.DB_PREFIX.$GLOBALS['_MODULE_DATAOBJECTS_DBPREFIX'].' ADD '.$identifier.' VARCHAR(10) NOT NULL';
		$result_pointer = $db_connection->query($query);
	}	
...
##
## =======================================================================        
function clients_token_setupSearch($ctlData,$data,$element_count) {
...
##
## =======================================================================        
function clients_token_getSearchQuery($ctlData,$data) {
	if(isset($ctlData['SEARCHABLE'])) {
		if($ctlData['SEARCHABLE'] == 'no') {
			return;
		}
...
##
## =======================================================================        
function clients_token_storeSearchData($i) {
...
## =======================================================================        
##  clients_token_getExportData        
## =======================================================================        
##  retrieves the data for a certain field of a certain client
##
## =======================================================================        
function clients_token_getExportData($ctlData,$clientID) {	
	return clients_token_getData($ctlData,$clientID);
}



## =======================================================================        
##  clients_token_importData        
## =======================================================================        
##  imports the data for all elements of this attribute type in a data set
##
## =======================================================================        
function clients_token_importData($id,$ctlData,$data) {
	global $Auth,$gSession;

	## we are responsible to import all elements of our type
	$values = array();
	foreach($ctlData as $current_element) {
		if($current_element['TYPE'] == 'TOKEN' && isset($data[$current_element['IDENTIFIER']])) {
